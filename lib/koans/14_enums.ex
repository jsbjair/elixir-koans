defmodule Enums do
  ...
  koan "You three there, follow me!" do
    assert Enum.take([1, 2, 3, 4, 5], 3) == [1, 2, 3] # left_slice by size
  end

  koan "You can ask for a lot, but Enum won't hand you more than you give" do
    assert Enum.take([1, 2, 3, 4, 5], 10) == [1, 2, 3, 4, 5] # right_slice
  end

  koan "Just like taking, you can also drop elements" do
    assert Enum.drop([-1, 0, 1, 2, 3], 2) == [1, 2, 3] # slice_left fixed size
  end
